...
class DownController extends PublicController {
	...
	//导出已上报的体质信息兼容教育部系统的数据
	public function cshedu(){
		$ac = I('ac','');

		if(IS_POST && $ac == 'export'){

			if($this->school_code == '')$this->error('请选择学校！');
			
			$gradeItem = C('GRADE_ITEM');
			$gradeItemField = C('GRADE_ITEM_FIELD');
			$item_no = C('ITEM_NO');
			...
			*/		
			...
			foreach($data as $k=>$row){
				...
				//项目成绩
				$stuScoreList =  D('ItemScore')->get_info_list($row['partition_field'],$row['year_score_id']);

				if(empty($stuScoreList))$this->error('体质信息错误！');
				...
				foreach($stuScoreList as $k=>$v){
					switch($v['item_id']){
						...
						case '06':
						case '07':
							$v['exam_result_display'] = round($v['exam_result_display'],1);
							break;
						case '04':
						case '12':
						case '14':
		...
		$this->web_title = '兼容教育部体测模版数据导出';
		$this->page_template = "Down:cshedu";
	}
}
